...
int main()
{
	...
	do {
		...
		case 5:
			...
			cout << "\tВведіть оцінку з профільного предмету: "; cin >> profpred;
			...
			cout << endl;
			if ((found = BinSearch(s, N, profpred, kurs, secondname)) != -1)
			{
...
int BinSearch(Student* s, const int N, const int profpred, const int kurs, const string secondname)
{
	...
	do {
		m = (L + R) / 2;
		if (s[m].prog == profpred && s[m].kurs == kurs && s[m].secondname == secondname)
		{
			return m;
		}
		if ((s[m].prog < profpred)
			||
			(s[m].prog == profpred &&
			 s[m].kurs < kurs)
			||
...
int* IndexSort(Student* s, const int N)
{
	...
	{
		value = I[i];
		for (j = i - 1; j >= 0 && (s[I[j]].prog > s[value].prog) ||
